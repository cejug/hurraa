...
<body>
	...
	<div>
		<a href="${linkTo[SectorController].form}">
			 <fmt:message key="sector.form.insert" />
		</a>
	</div>
	<table border="1">
		<tr>
		...
		<c:forEach items="${sectors }" var="sector">
			<tr>
				<td>${sector.id }</td>
				<td>${sector.name }</td>
				<td><a href="${linkTo[SectorController].form(sector.id) }"> <fmt:message
							key="label.update" />
				</a></td>
				<td><a href="${linkTo[SectorController].delete(sector) }"> <fmt:message
							key="label.remove" />
				</a></td>
			</tr>
		</c:forEach>
